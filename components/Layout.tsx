...
const Layout: React.FC<LayoutProps> = ({ user, children, activeTab, setActiveTab, logout }) => {
  ...
  const menuItems = [
    { id: 'dashboard', label: 'Dashboard', icon: '📊', roles: [UserRole.STUDENT, UserRole.TEACHER, UserRole.ADMIN] },
    { id: 'assignments', label: 'Assignments', icon: '📝', roles: [UserRole.STUDENT, UserRole.TEACHER] },
    { id: 'library', label: 'Library', icon: '📚', roles: [UserRole.STUDENT, UserRole.TEACHER, UserRole.ADMIN] },
    { id: 'grades', label: 'Grades', icon: '⭐', roles: [UserRole.STUDENT, UserRole.TEACHER] },
    { id: 'tutor', label: 'AI Tutor', icon: '🤖', roles: [UserRole.STUDENT] },
    { id: 'admin', label: 'Management', icon: '⚙️', roles: [UserRole.ADMIN] },
    { id: 'profile', label: 'Profile', icon: '👤', roles: [UserRole.STUDENT, UserRole.TEACHER, UserRole.ADMIN] },
  ];
    ...
    <>
      <div className="p-6">
        <h1 className="text-2xl font-bold flex items-center gap-2">
          ...
          <span className="flex">
            <span class="text-indigo-600 dark:text-indigo-400">Study</span>
            <span className="text-sky-400 ml-1">Link</span>
          </span>
        </h1>
      </div>
        ...
        <header className="h-16 bg-white dark:bg-slate-900 border-b border-gray-200 dark:border-slate-800 flex items-center justify-between px-6 md:px-12 transition-colors">
          ...
          <div className="flex items-center gap-4 sm:gap-6">
            ...
            <span className="hidden sm:inline-block text-xs font-medium px-3 py-1 bg-sky-100 dark:bg-sky-900/40 text-sky-700 dark:text-sky-300 rounded-full">
              Level: {user.level || 'B2 Upper Intermediate'}
            </span>
          </div>
        </header>
